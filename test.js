...
async function runTests() {
  ...
  if (referrals.referrals.length !== 1) throw new Error('测试6失败: 应有1个推荐人');
  ...
  if (network.level1.paid !== 1) throw new Error('测试8失败: 应有1个付费一级推荐');

  // 测试9: 申请提现（金额不足100，应失败）
  console.log('\n📝 测试9: 申请提现（测试最低金额限制）');
  ...
  if (!withdraw.error.includes('100')) throw new Error('测试9失败: 应提示最低100元');
